...
from torleechbot.helper_funcs.download_aria_p_n import aria_start
from torleechbot.helper_funcs.youtube_dl_button import youtube_dl_call_back
from torleechbot.plugins.status_message_fn import cancel_message_f
                ...
                if len(g_del_list) != 0:
                    for f in g_del_list:
                        if os.path.isfile(f):
                            ...
                            shutil.rmtree(f)
                    await update.message.edit_text(f"You just Deleted {len(g_del_list)} objects 😬")
                else:
